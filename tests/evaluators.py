...
from coreason_deep_research.utils import get_today_str
...
def _format_input_query(inputs: dict) -> str:
    ...
    return "\n\n".join(
        [role_to_string_format_map[message["role"]].format(content=message["content"]) for message in messages]
    )
...
def eval_overall_quality(inputs: dict, outputs: dict):
    ...
    if isinstance(eval_model, ChatAnthropic):
        user_input_content = [
            {"type": "text", "text": user_input_content, "cache_control": {"type": "ephemeral", "ttl": "1h"}}
        ]
    eval_result = cast(
        "OverallQualityScore",
...
def eval_structure(inputs: dict, outputs: dict):
    ...
    eval_result = cast(
        "StructureScore",
        eval_model.with_structured_output(StructureScore).invoke([{"role": "user", "content": user_input_content}]),
    )
...
def eval_correctness(inputs: dict, outputs: dict, reference_outputs: dict):
    ...
    eval_result = cast(
        "CorrectnessScore",
        eval_model.with_structured_output(CorrectnessScore).invoke([{"role": "user", "content": user_input_content}]),
    )
...
def eval_completeness(inputs: dict, outputs: dict):
    ...
    eval_result = cast(
        "CompletenessScore",
        eval_model.with_structured_output(CompletenessScore).invoke([{"role": "user", "content": user_input_content}]),
    )
