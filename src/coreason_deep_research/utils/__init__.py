...
from coreason_deep_research.configuration import Configuration, SearchAPI
from coreason_deep_research.prompts import summarize_webpage_prompt
from coreason_deep_research.state import ResearchComplete, Summary
...
async def summarize_webpage(model: BaseChatModel, webpage_content: str) -> str:
    ...
    try:
        ...
        logging.warning(f"Summarization failed with error: {e!s}, returning original content")
        return webpage_content
...
async def load_mcp_tools(
    ...
    for mcp_tool in available_mcp_tools:
        ...
        if mcp_tool.name in existing_tool_names:
            warnings.warn(f"MCP tool '{mcp_tool.name}' conflicts with existing tool name - skipping", stacklevel=2)
            continue
